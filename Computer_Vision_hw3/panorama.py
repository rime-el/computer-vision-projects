...
def harris_corners(img, window_size=3, k=0.04):
    ...
    ### YOUR CODE HERE
    dx_squr = convolve(dx ** 2, window, mode = 'constant')
    dy_squr = convolve(dy ** 2, window, mode = 'constant')
    trace = dx_squr + dy_squr
    dxdy = convolve(dx * dy, window, mode  = 'constant')
    det = dx_squr * dy_squr - dxdy ** 2
...
def simple_descriptor(patch):
    ...
    feature = []
    ### YOUR CODE HERE
    standart_deviation = patch.std()
    if standart_deviation == 0:
        standart_deviation = 1
    mean = patch.mean()
    feature = ((patch - mean)/standart_deviation).reshape(-1)
    ### END YOUR CODE
    return feature
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for x in range(n_iters):
        random_set = np.random.choice(N, n_samples, replace = False)
        a = matched1[random_set]
        b = matched2[random_set]
        H = affine_matrix(a, b)
        inliers = np.sqrt(((matched2.dot(H) - matched1)**2).sum(axis=-1)) < threshold
        largest_inliers = inliers.sum()
        if largest_inliers > n_inliers:
            max_inliers = inliers
            n_inliers = largest_inliers 
    H = affine_matrix(matched2[max_inliers], matched1[max_inliers])
    ### END YOUR CODE
    print(H)
