...
def hog_feature(image, pixel_per_cell=8):
    ...
    hog_feature, hog_image=feature.hog(
        image, pixels_per_cell=(pixel_per_cell,pixel_per_cell), 
        block_norm ='L1',visualize=True, feature_vector=True
    )
    ### END YOUR CODE
...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            ...
            if window.shape != (winH, winW):
                continue
            window_feature, _ = feature.hog(
                window,pixels_per_cell = (pixel_per_cell, pixel_per_cell),
                block_norm = 'L1',visualize =True,feature_vector =True
            )
            score = np.dot(window_feature, template_feature)
            if score > max_score:
                max_score = score
                maxr = r
                maxc = c
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    for current_scale, scaled_image in images:
        score, r, c, response_map_resized = sliding_window(
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    for heatmap_part, sigma in zip(heatmaps, sigmas):
        filtered_heatmap = gaussian(heatmap_part, sigma)
